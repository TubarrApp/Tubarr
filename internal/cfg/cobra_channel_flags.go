...
func setPrimaryChannelFlags(cmd *cobra.Command, url, name *string, id *int) {
	if url != nil {
...
func setFileDirFlags(cmd *cobra.Command, vDir, jDir *string) {
...
func setProgramRelatedFlags(cmd *cobra.Command, concurrency, crawlFreq *int, downloadCmd, downloadArgs *string) {
	cmd.Flags().IntVarP(concurrency, keys.Concurrency, "l", 0, "Maximum concurrent videos to download/process for this channel")
	cmd.Flags().IntVar(crawlFreq, "crawl-freq", 0, "New crawl frequency in minutes")
...
func setDownloadFlags(cmd *cobra.Command, maxFilesize *string, cookieSource *string, retries *int, dlFilters *[]string) {
...
func setMetarrFlags(cmd *cobra.Command, ext *string, maxCPU *float64, metarrConcurrency *int, minFreeMem, outDir, renameStyle *string, fileSfxReplace, metaOps *[]string, filenameDateTag *string) {
